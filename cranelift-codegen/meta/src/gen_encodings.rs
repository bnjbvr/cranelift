...
impl Level2Table {
    ...
    fn layout_hashtable(
        ...
        level2_doc: &mut DefaultMap<usize, Vec<String>>,
    ) {
        let hash_table = generate_table(
            self.inst_to_encodings.values(),
            self.inst_to_encodings.len(),
            |enc_list| enc_list.inst.opcode_number.index(),
        );
...
impl EncodingList {
    ...
    fn encode(
        ...
        enc_lists_doc: &mut DefaultMap<usize, Vec<String>>,
    ) {
...
fn emit_tables(defs: &SharedDefinitions, isa: &TargetIsa, fmt: &mut Formatter) {
    ...
    let mut level2_doc: DefaultMap<usize, Vec<String>> = DefaultMap::new();

    // Tables for encoding lists with comments.
    let mut enc_lists = UniqueSeqTable::new();
    let mut enc_lists_doc = DefaultMap::new();
